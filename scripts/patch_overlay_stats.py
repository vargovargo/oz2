...
def load_parquets():
    ...
    return dci, pp, nmtc, elig, cra


def compute_stats(dci, pp, nmtc, elig, cra=None):
...
OVERLAY_FIELDS = ("dci_q1_tracts", "pp_county_count", "nmtc_projects", "cra_lmi_tracts")
...
def ensure_yaml_fields() -> None:
    ...
    while i < len(lines):
        ...
        # Use null (not 0) so the state page card stays hidden until real data
        # is loaded via ingest_cra_lmi.py + patch_overlay_stats.py.
    ...
    if inserted_cra > 0 or inserted_all > 0:
...
def main():
    ...
    dci, pp, nmtc, elig, cra = load_parquets()
    ...
    stats = compute_stats(dci, pp, nmtc, elig, cra)
